	...
	<div class="ts-main-content">
		...
		<div class="content-wrapper">
			...
			<div class="col-md-12">
						...
						<h2 class="page-title">Báo cáo chi phí bảo hiểm</h2>
            </div>
            <form id="filter-form">
						...
						<div class="panel panel-default">
							...
							<?php if($error){?><div class="errorWrap"><strong>Lỗi</strong>:<?php echo htmlentities($error); ?> </div><?php }
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php $sql = "SELECT thongtinxe.TenXe,baohiem.*  
									from baohiem 
									join thongtinxe on thongtinxe.id=baohiem.idxe 
                                    where date(baohiem.NgayMua) between '{$date_start}' and '{$date_end}' $bike";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
{		$total=$total+$result->SoTien;	?>
    ...
    $(function(){
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "report-expenses.php?date_start="+$('[name="date_start"]').val()+"&date_end="+$('[name="date_end"]').val()+"&xethue="+$('[name="xethue"]').val()
        })
    })
    </script>
    	<script>
