...
?>
            ...
            <div class="row">
            <h4 class="panel-heading text-center text-bold">Báo cáo doanh thu của <?php echo htmlentities($b);?></h4>
            </div>
						...
						<div class="panel panel-default">
							...
							<?php if($error){?><div class="errorWrap"><strong>Lỗi</strong>:<?php echo htmlentities($error); ?> </div><?php }
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php $sql = "SELECT khachhang.HoVaTen,khachhang.CCCD,khachhang.SoDienThoai,hangxe.TenHang,thongtinxe.TenXe,thongtinxe.GiaThueTheoNgay,dathang.NgayThue,dathang.NgayTra,dathang.GhiChu,dathang.idxe as vid,dathang.TinhTrang,dathang.NgayNhap,dathang.GiaTriHopDong,dathang.SoNgayThue,dathang.id  
									from dathang 
                                    ...
                                    where dathang.TinhTrang=1 and date(dathang.NgayTra) between '{$date_start}' and '{$date_end}' $bike";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
foreach($results as $result)
{		$total=$total+$result->GiaTriHopDong;	?>
										<tr>
											<td><?php echo htmlentities($cnt);?></td>
											<td>
                                                <small><span class="text-muted">Tên: </span><?php echo htmlentities($result->HoVaTen) ?></small><br>
                                                <small><span class="text-muted">CCCD: </span><?php echo htmlentities($result->CCCD) ?></small><br>
                                                <small><span class="text-muted">SĐT: </span><?php echo htmlentities($result->SoDienThoai) ?></small>
                                            </td>
											<td>
                                                <small><span class="text-muted">Biển số: </span><?php echo htmlentities($result->TenXe) ?></small><br>
                                                <small><span class="text-muted">Hãng: </span><?php echo htmlentities($result->TenHang) ?></small><br>
                                                <small><span class="text-muted">Giá thuê: </span><?php echo htmlentities(($result->GiaTriHopDong)/($result->SoNgayThue)) ?></small>
                                            </td>
    ...
    $(function(){
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "report-sales.php?date_start="+$('[name="date_start"]').val()+"&date_end="+$('[name="date_end"]').val()+"&xethue="+$('[name="xethue"]').val()
        })
    })
    </script>
    	<script>
